...
app.post('/api/auth/login', (req, res) => {
  ...
  const user = db.users.find((u) => {
    ...
    return (
      ...
      (inputEmail === 'owner@nabsite.et' && u.role === 'OWNER') ||
      (inputEmail === 'owner@nabsite.io' && u.role === 'OWNER') ||
      (inputEmail === 'owner' && u.role === 'OWNER') ||
...
// Hidden Owner Gateway Login (/owner/gateway or /platform-access)
app.post('/api/auth/owner-login', (req, res) => {
  ...
  db.log(owner.id, owner.name, 'OWNER', 'OWNER_COMMAND_ACCESS', 'PLATFORM', 'owner_gateway', undefined, 'SUCCESS');
