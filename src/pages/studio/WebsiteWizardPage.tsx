...
export const WebsiteWizardPage: React.FC = () => {
  ...
  const [selectedThemeId, setSelectedThemeId] = useState<string>('theme_corporate');
    ...
    api.getCompany(id)
      .then((res) => {
        if (res?.company) {
          ...
          // Suggest appropriate theme based on category
          const cat = res.company.category?.toLowerCase() || '';
          if (cat.includes('food') || cat.includes('restaurant') || cat.includes('cafe')) {
            setSelectedThemeId('theme_restaurant');
          } else if (cat.includes('tech') || cat.includes('software')) {
            setSelectedThemeId('theme_technology');
          } else if (cat.includes('hotel') || cat.includes('hospitality')) {
            setSelectedThemeId('theme_hotel');
          } else if (cat.includes('retail') || cat.includes('shop') || cat.includes('boutique')) {
            setSelectedThemeId('theme_retail');
          }
        }
      })
      .catch(console.error)
      ...
      {step === 2 && (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">
                Select Theme Archetype (24 Available)
              </h2>
              <p className="text-xs text-slate-500">
                Pick the visual foundation that best captures {company.name}'s identity.
              </p>
            </div>
            <span className="text-xs font-bold text-amber-600 bg-amber-50 dark:bg-amber-950/40 px-3 py-1 rounded-full">
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[9px] uppercase font-bold px-1.5 py-0.5 rounded-sm bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300">
                      {t.categoryCompatibilities?.[0] || 'Universal'}
                    </span>
                  </div>
