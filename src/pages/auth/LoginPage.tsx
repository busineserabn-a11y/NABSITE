...
export const LoginPage: React.FC = () => {
  ...
  const handleDemoLogin = async (demoEmail: string) => {
    ...
    try {
      ...
      if (demoEmail.toLowerCase().includes('owner')) {
        navigate('/owner');
      } else {
        navigate('/dashboard');
      }
        ...
        <Card variant="bordered" padding="lg" className="space-y-6 shadow-xl">
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-slate-800 space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              <button
                type="button"
                onClick={() => handleDemoLogin('owner@nabsite.io')}
                className="p-2.5 text-left rounded-xl border border-amber-300 dark:border-amber-700 bg-amber-50/50 dark:bg-amber-950/20 hover:bg-amber-100/50 transition-colors"
              >
                <p className="text-xs font-bold text-amber-900 dark:text-amber-300">👑 Owner</p>
                <p className="text-[10px] text-amber-700/80 dark:text-amber-400">Master God Mode</p>
              </button>
              ...
              >
                ...
                <p className="text-[10px] text-slate-500">Multi-company</p>
              </button>
